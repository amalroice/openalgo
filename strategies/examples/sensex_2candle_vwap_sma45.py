"""SENSEX two-candle VWAP + SMA(45) breakout, ATM weekly BFO options.

A copy of the NIFTY two-candle strategy (strategies/examples/
nifty_2candle_vwap_sma45.py), re-tuned for SENSEX on 2026-09-21. Signals are
read off SENSEX SPOT (BSE_INDEX) on 5-minute candles; orders go to the ATM
option of the nearest weekly BFO expiry - CE on a long signal, PE on a short.
Every stop and trail is measured in INDEX points, never in premium.
...
Entry (long; short is the mirror):
    ...
    3. Candle 2 closes above candle 1's HIGH (short: below its LOW).
...
Stop:
    Candle 1's low less STOP_BUFFER (50) points, or the higher line less the
    buffer when that line sits within LINE_PROXIMITY (82) of the level.

Management:
    trail      the stop sits TRAIL_DIST_PCT of the entry behind the best
               price, moved in TRAIL_STEP_PCT steps.
    ...
    broker     a resting stop-limit at the broker is the mechanical-failure
               backstop only.
    15:00      everything is squared off

Backtest, 2016-10..2026-09, SENSEX 5m from Angel, net of 10 index points a
trade (a scaled estimate, not a measured SENSEX option cost), exits at the bar
close. Rules tuned on 2016-22 only, then checked on 2023-26:

                                  trades  win    PF    maxDD   losing years
    base (10:30-14:30, buffer 33)  1,722  37%   1.09  -4,561        3
    this script                      955  44%   1.23  -2,631        2

    2016-22 PF 1.01 -> 1.11; 2023-26 PF 1.20 -> 1.41 (win 47%).
    Year by year: 2017 -703, 2018 +704, 2019 +483, 2020 +1,264,
    2021 +2,172, 2022 -560, 2023 +814, 2024 +3,912, 2025 +3,633,
    2026 to Sep +820.

Neighbouring settings (buffer 40/65, slope over 3/12 bars, other lock levels)
all land at PF 1.19-1.23, so none of the four is a knife-edge fit. Breakeven
cost is about 18 points a trade; at 14 the PF is 1.16. Option decay is not in
these figures. Reproduce: session 684aa630 scratchpad sensex/eng2.py.

SENSEX spot publishes no volume, so the VWAP is weighted by synthetic volume:
the summed NSE traded shares of the 30 SENSEX constituents on the same
5-minute grid, applied to the INDEX typical price (H+L+C)/3. NSE volume is a
proxy for the BSE listing - the backtest used the same proxy.

SAFETY: DRY_RUN = True places nothing at all - entries, exits and the broker
stop are only logged, with the option's real premium, so the log is a paper
record. With DRY_RUN = False the script refuses to place an order unless
OpenAlgo is in analyzer (sandbox) mode or I_UNDERSTAND_THIS_IS_LIVE is True.
...
# Paper first: DRY_RUN logs every order it would place, with the option's real
# premium, and sends nothing. Analyzer mode is a GLOBAL instance flag and the
...
EQUITY_EXCHANGE = "NSE"             # constituent volume proxy, as backtested
FO_EXCHANGE = "BFO"
...
LOTS = 1                            # SENSEX lot is 20

# Percent trailing stop, carried over from NIFTY unchanged. On SENSEX 0.6%/0.2%
# and 0.4%/0.1% land within PF 0.02 of it.
TRAIL_DIST_PCT = 0.005              # stop distance behind the best price
TRAIL_STEP_PCT = 0.002              # the stop moves once per step of this size

# The NIFTY copy carries a switched-off target and close stop; both backtested
# badly there and are left off here. The code paths stay so the two copies do
# not drift apart.
TARGET_R = None
CLOSE_STOP = False
TARGET_POLL_SECONDS = 5

# Profit lock: once the best move reaches LOCK_AT_R x the initial risk (entry
# to the buffered stop), the stop is at least LOCK_TO_R x risk in profit. On
# SENSEX it lifts the win rate 2-3 points (41.6 -> 43.2% in 2016-22, 43.5 ->
# 46.6% in 2023-26); profit and drawdown are about unchanged. None disables it.
LOCK_AT_R = 1.5
LOCK_TO_R = 0.5
...
# Resting stop parked at the broker on entry - see the NIFTY copy for why the
# index stop is converted with delta AND gamma and then widened by theta and
# vega. Checked 2026-09-21: Angel's BFO greeks come back in the same shape as
# NFO. The BFO stop-limit itself has never been placed; watch the first live
# entry for "broker stop resting" versus "broker stop REJECTED".
...
TICK = 0.05                         # BFO option tick

# No VIX filter on SENSEX: VIX 10+ moved the 10:30-14:30 result from +8,643 to
# +8,225, i.e. nothing. None keeps the gate off.
VIX_MIN = None

# Trend filters, 2026-09-21. SENSEX replay, PF shown as tuned 2016-22 /
# checked 2023-26:
#
#     base (buffer 33)                   PF 1.01 / 1.20
#     + with the day                     PF 1.09 / 1.23   the largest single gain
#     + with the day + buffer 50         PF 1.12 / 1.30
#     + ... + slope over 6               PF 1.12 / 1.40
#
# WITH_THE_DAY: a long needs candle 2 to close above today's first open, a
...
# 65 land within PF 0.04 of it.
STOP_BUFFER = 50.0                  # stop sits this far beyond candle 1
LINE_PROXIMITY = 82.0               # swap to the line when it is this close

SESSION_OPEN = dtime(9, 15)
NO_NEW_ENTRY_BEFORE = dtime(10, 30)  # plain 09:15-15:00 entries break even
...
USE_WEBSOCKET_VOLUME = True         # False falls back to 31 history calls a bar
...
STATE_FILE = STATE_DIR / "sensex_2candle_vwap_sma45.json"

# SENSEX 30 constituents as of the June 2025 BSE review (TRENT and BEL in for
# NESTLEIND and INDUSINDBK), traded on NSE under the same tickers. TMPV is Tata
# Motors after the passenger-vehicle demerger. Re-verify at every BSE review.
CONSTITUENTS = [
...
def latest_signal(frame: pd.DataFrame) -> dict | None:
    ...
    for direction, flag in (("long", "above"), ("short", "below")):
        ...
        # was not the intended rule. Replayed 2016-10..2026-09 on the NIFTY copy's
        ...
        if WITH_THE_DAY:
            ...
            if not (close > day_open if direction == "long" else close < day_open):
                log(f"{direction} signal skipped: close {close:.2f} is against "
                    f"today's open {day_open:.2f}")
                continue
        if SLOPE_BARS:
...
def nearest_weekly(client) -> str | None:
    ...
    0-DTE contract cannot express the move the backtest measured. SENSEX expires
    weekly too, so this costs one week of extra time value about one day in five.
...
def send(client, symbol: str, action: str, quantity: int) -> bool:
    ...
    """
    if DRY_RUN:
        premium = option_ltp(client, symbol)
        shown = f"{premium:.2f}" if premium is not None else "unknown"
        log(f"DRY_RUN {action} {quantity} {symbol} at premium {shown}")
        return True
...
def option_ltp(client, symbol: str) -> float | None:
    """Last traded premium of one option leg, for the DRY_RUN paper record.

    Returns:
        The premium, or None when the quote cannot be read.
    """
    try:
...
def place_broker_stop(client, symbol: str, quantity: int,
    ...
    if DRY_RUN:
        # A resting SELL with no position behind it would be a naked short.
        log(f"DRY_RUN broker stop: {quantity} {symbol} trigger {trigger:.2f} "
            f"limit {limit:.2f} (not placed)")
        return None
    try:
        r = client.placeorder(
